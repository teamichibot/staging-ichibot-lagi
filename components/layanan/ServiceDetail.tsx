...
export function ServiceDetail({ service }: { service: ServiceData }) {
  ...
  return (
    <main className="min-h-screen bg-[#050A14] relative overflow-hidden">
      ...
      <div className="absolute top-0 left-[-10%] w-[600px] h-[600px] bg-teal/5 rounded-full blur-[120px] pointer-events-none" />
      <div className="absolute bottom-0 right-[-10%] w-[500px] h-[500px] bg-navy/20 rounded-full blur-[100px] pointer-events-none" />
      ...
      <section className="relative bg-[#050A14] overflow-hidden pt-32 pb-24 md:pt-40 md:pb-32 border-b border-white/5">
        ...
        />
        <div className="absolute top-20 right-20 w-[500px] h-[500px] bg-teal/15 rounded-full blur-[100px] pointer-events-none" />
        ...
        <div className="relative max-w-4xl mx-auto px-6 reveal">
          ...
          <span className="block text-teal text-xs font-bold uppercase tracking-[0.2em] mb-6">
            {lang === 'id' ? 'Solusi & Layanan' : 'Solutions & Services'}
          </span>
      ...
      <div className="max-w-5xl mx-auto px-6 pb-32 relative z-10">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {service.benefits.map((benefit, i) => (
              <div key={i} className="flex items-start gap-6 p-7 rounded-[2rem] glass-3d-premium group hover:border-teal/30 transition-all duration-500">
                ...
                <p className="text-slate-300 text-base leading-relaxed pt-1 opacity-90">{tx(benefit)}</p>
              </div>
            ))}
          </div>
        </section>
        ...
        <section className="mb-24 reveal" style={{ animationDelay: '400ms' }}>
          ...
          <div className="relative pl-4">
            <div className="absolute left-9 top-0 bottom-0 w-px bg-white/10 hidden md:block" />
            <div className="space-y-12">
              {service.process.map((step, i) => (
                <div key={i} className="flex gap-8 relative items-start group">
                  <div className="flex-none w-10 h-10 rounded-full bg-navy border border-teal/30 flex items-center justify-center text-teal text-sm font-bold z-10 shrink-0 group-hover:scale-110 group-hover:bg-teal group-hover:text-navy transition-all shadow-[0_0_15px_rgba(45,212,191,0.2)]">
                    {i + 1}
                  </div>
                  <div className="flex-1 pb-8 border-b border-white/5 last:border-0">
                    <h3 className="font-display text-xl font-bold text-white mb-3 tracking-tight group-hover:text-teal transition-colors">{tx(step.title)}</h3>
                    <p className="text-slate-400 text-base leading-relaxed opacity-80 group-hover:opacity-100 transition-opacity">{tx(step.desc)}</p>
                  </div>
                </div>
              ))}
            </div>
        ...
        <section className="mb-24 reveal" style={{ animationDelay: '500ms' }}>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {service.useCases.map((uc, i) => (
              <div key={i} className="p-8 rounded-[2rem] glass-3d-premium group hover:border-teal/30 transition-all duration-500 relative overflow-hidden flex flex-col gap-5">
                <div className="absolute top-0 right-0 w-24 h-24 bg-teal/5 rounded-full blur-2xl group-hover:bg-teal/10 transition-colors" />
                <div className="w-12 h-12 rounded-xl bg-white/5 flex items-center justify-center group-hover:scale-110 transition-transform">
                  <svg className="w-6 h-6 text-teal" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13 10V3L4 14h7v7l9-11h-7z" />
                  </svg>
                </div>
                <h3 className="font-display text-lg font-bold text-white tracking-tight">{tx(uc.title)}</h3>
                <p className="text-slate-400 text-sm leading-relaxed opacity-80 group-hover:opacity-100 transition-opacity">{tx(uc.desc)}</p>
              </div>
            ))}
          </div>
        </section>
        ...
        <section className="rounded-[3rem] glass-3d-premium p-12 md:p-16 text-center reveal shadow-2xl" style={{ animationDelay: '600ms' }}>
          ...
          <div className="flex flex-wrap justify-center gap-6">
            ...
            <a
              ...
              target="_blank"
              rel="noopener noreferrer"
              className="inline-flex items-center gap-3 bg-white/5 hover:bg-white/10 border border-white/10 text-white font-bold px-10 py-5 rounded-2xl transition-all text-base"
            >
