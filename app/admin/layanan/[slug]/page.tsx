  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
        <div>
          <span className="text-xs text-ink/40 mb-1 block">Indonesia</span>
          <input value={value.id} onChange={(e) => onChange({ ...value, id: e.target.value })} className="input-field w-full" />
        </div>
        <div>
          <span className="text-xs text-ink/40 mb-1 block">English</span>
...
function ArrayItemField({ label, items, onChange }: { label: string; items: ArrayItem[]; onChange: (v: ArrayItem[]) => void }) {
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-2">
        <label className="text-sm font-medium text-ink/85">{label}</label>
        <button type="button" onClick={() => onChange([...items, empty])} className="text-xs text-brand hover:underline">+ Tambah</button>
      </div>
      <div className="space-y-4">
        {items.map((item, i) => (
          <div key={i} className="border border-black/10 rounded-xl p-4 relative">
            ...
            <p className="text-xs font-semibold text-ink/55 mb-3">Item {i + 1}</p>
      ...
      <div className="p-6 md:p-8 max-w-3xl">
        <div className="flex items-center justify-between mb-8">
          <div className="flex items-center gap-3">
            <button onClick={() => router.push('/admin/layanan')} className="text-ink/40 hover:text-ink/65 transition-colors">
              <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 19l-7-7 7-7" /></svg>
            </button>
            <h1 className="text-2xl font-bold text-ink">{isNew ? 'Tambah Layanan' : 'Edit Layanan'}</h1>
          </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="flex gap-3">
            ...
            <button type="button" onClick={() => router.push('/admin/layanan')} className="px-6 py-2.5 rounded-xl text-sm font-medium text-ink/65 hover:bg-black/5 transition-colors">
              Batal
            </button>
          </div>
        </form>
