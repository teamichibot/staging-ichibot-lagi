  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-2 gap-2">
        <div><span className="text-xs text-ink/40 mb-1 block">Indonesia</span>
          <input value={value.id} onChange={(e) => onChange({ ...value, id: e.target.value })} className="input-field w-full" /></div>
        <div><span className="text-xs text-ink/40 mb-1 block">English</span>
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-2">
        ...
        <button type="button" onClick={() => onChange([...items, { id: '', en: '' }])} className="text-xs text-brand hover:underline">+ Tambah</button>
      </div>
      <div className="space-y-2">
        {items.map((item, i) => (
          <div key={i} className="flex gap-2 items-start">
            ...
            <button type="button" onClick={() => onChange(items.filter((_, j) => j !== i))} className="text-ink/40 hover:text-red-500 mt-2">
...
function ArrayFeatureField({ items, onChange }: { items: FeatureItem[]; onChange: (v: FeatureItem[]) => void }) {
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-2">
        <label className="text-sm font-medium text-ink/85">Fitur Utama</label>
        <button type="button" onClick={() => onChange([...items, emptyItem])} className="text-xs text-brand hover:underline">+ Tambah</button>
      </div>
      <div className="space-y-4">
        {items.map((item, i) => (
          <div key={i} className="border border-black/10 rounded-xl p-4 relative">
            <button type="button" onClick={() => onChange(items.filter((_, j) => j !== i))} className="absolute top-3 right-3 text-ink/40 hover:text-red-500">
              <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12" /></svg>
            </button>
            <p className="text-xs font-semibold text-ink/55 mb-3">Fitur {i + 1}</p>
...
function ArraySpecField({ items, onChange }: { items: SpecItem[]; onChange: (v: SpecItem[]) => void }) {
  return (
    <div>
      <div className="flex items-center justify-between mb-2">
        <label className="text-sm font-medium text-ink/85">Spesifikasi Teknis</label>
        <button type="button" onClick={() => onChange([...items, { label: { id: '', en: '' }, value: { id: '', en: '' } }])} className="text-xs text-brand hover:underline">+ Tambah</button>
      </div>
      <div className="space-y-2">
        {items.map((item, i) => (
          <div key={i} className="flex gap-2 items-start">
      ...
      <div className="p-6 md:p-8 max-w-3xl">
        <div className="flex items-center justify-between mb-8">
          <div className="flex items-center gap-3">
            <button onClick={() => router.push('/admin/produk')} className="text-ink/40 hover:text-ink/65 transition-colors">
              <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 19l-7-7 7-7" /></svg>
            </button>
            <h1 className="text-2xl font-bold text-ink">{isNew ? 'Tambah Produk' : 'Edit Produk'}</h1>
          </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          <div className="bg-white rounded-2xl border border-black/10 p-6 space-y-5">
            <div className="flex items-center justify-between">
              <h2 className="font-semibold text-ink text-sm uppercase">Informasi Dasar</h2>
              <span className="text-[10px] bg-brand/10 text-brand px-2 py-0.5 rounded font-bold">Auto-Translate Active</span>
            </div>
            <div className="grid grid-cols-2 gap-4">
              <div>
                <label className="block text-sm font-medium text-ink/85 mb-1.5">Slug</label>
                <input value={form.slug} onChange={(e) => set('slug', e.target.value)} required className="input-field w-full" />
              </div>
              <div>
                <label className="block text-sm font-medium text-ink/85 mb-1.5">URL Gambar</label>
          ...
          <div className="flex gap-3">
            ...
            <button type="button" onClick={() => router.push('/admin/produk')} className="px-6 py-2.5 rounded-xl text-sm font-medium text-ink/65 hover:bg-black/5 transition-colors">
              Batal
            </button>
          </div>
        </form>
